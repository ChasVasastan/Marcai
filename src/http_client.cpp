...
void http::client::tls_tcp_setup(http::request *req, const ip_addr_t &addr)
{
    // Set up TLS
...
void http::client::dns_resolve_callback(const char *name, const ip_addr_t *addr, void *arg)
{
    http::request *req = reinterpret_cast<http::request*>(arg);
    if (addr != NULL)
    {
